...
We propose a novel intrusion detection algorithm that just requires a minor modification of the OTA update protocol by enabling an \dme{active message}{What's an ``active message''? This has to be defined.} once the update has been completed.
...
The system is aimed at detecting deviation from expected behaviour
...
\begin{equation}
\label{eqn2} 
f_2 = \sum \limits_{i=0}^{n} \frac{\left( \mu_i - t_i \right)}{\left( \sigma_i + 1 \right)}
\end{equation}

where, \\
\hspace {2cm} $\mu_i$ - Mean Time at Mote \emph{i} \\
\hspace {2cm} $\sigma_i$ - Standard Deviation of  Time at Mote \emph{i} \\
...
\dme{An attacker is computationally bounded.}{This is interesting discussion, but not really relevant here AFAICS.} This is a standard
...
\dme{Most current standard security}{same comment regarding relevance / link of text}
...
Nodes within the event radius of a WSN have fair access to the wireless medium; this property is ensured by established protocols e.g. 802.11b and other variants of MAC layer protocols.
%
\dme{We can identify threats}{This is the beginnings of what you should be talking about, but the event-based/role-based presentation is a distraction. So far I don't think any unfamiliar reader will understand your design at this point.} based on expected outcome (probability) of transmission originated from each specific node. \\ %802.15.4 is the proposed WSN protocol
\begin{itemize}
...
\dme{It is possible to combine symmetric key and asymmetric key}{relevance?} cryptographic approaches to deny information to such nodes in the short run and/or policy based approaches in the long run. We shall examine presently implemented security techniques in WSN and shall improve upon them by blending cryptographic approaches at lower level (MAC/Network Layer) and role/event based approaches in higher layers, e.g. presentation layer.



\section{Implemetation of the Experiment}  %Methodology
\label{sec:imp}
\dme{PacketLink}{??} is most reliable when software acknowledgements are enabled 
