...
Trickle addresses single packet dissemination and uses suppression and dynamic adjustment of the broadcast rate to limit transmissions among neighbouring nodes.
...
\hline
\end{tabular}
\notedme{Why is this interesting or relevant? It's the scientific properties of the topologies that will count for something.}
...
For most experiments, the radio transmission power level was set at 100\% which has an approximate maximum effective transmission range of 50 meter and is able to interfere other motes up to 100 meter away.
...
\notedme{The `mote output' window aspect probably isn't interesting. It is worth mentioning that you have a means to collect the output from all of the serial ports of the simulated motes though.}
...
The new version of the image was transmitted to all motes in a fashion as determined by the \notedme{Check all instances of Deluge have uppercase initial letter} deluge and other associated protocols. \notedme{other protocols?}
...
\notedme{It's Bash or \texttt{bash} I'd say, but I'd not mention Bash anyway. Just say what you did with the data.}
...
\notedme{I'd say that step three is probably worth a single line of text, rather than what's here---but OTOH it's good that you're explaining each step! Indeed, if you've enumerated the different steps, how about using an actual enumeration environment in the \LaTeX?}
\begin{verbatim}
---------------------------------
| 	Time (in ms)	|	Mote ID	|
---------------------------------
...
\notedme{Step 4 is also probably only worth a line of text. Unless there's something non-trivial that you needed to implement, I'd avoid mentioning your C++. It sounds like here all you need to say is that you added a column containing the Euclidean distance from the mote to the sink?}
...
Processed data from multiple simulation runs initiated from the sink \notedme{previous text could be just as brief, but clearer. E.g.~what are you initiating? (I know, but reviewers might forget) Precision-wise, it's not really the \emph{run} you're initiating from the sink---the run includes the sink doing the initiating...} were considered to build the baseline data for each topology.
Any change in the topology or variables in the environment setting would necessitate recalibrating the baseline.
We considered 20 individual simulation runs initiated from the sink. \notedme{partial repetition?}
The processed data from all these runs were statistically treated to find out the `mean' and `standard deviation' of update time recorded at each of the nodes. \notedme{just say you calculated the arithmetic mean and standard deviation---currently the writing makes it sound unnecessarily complex.}
Both `mean' and `standard deviation' were used as input to the IDS to calculate the IWS according to the utility function shown in Equation-\ref{eqn2}. \notedme{Don't quote mean and standard deviation: they're just English.}\notedme{Why are you putting hyphens between Equation and the number? Should just be a space, no? (Possibly a non-breaking space, which is ~ in \LaTeX)}

%Step 6: Calculating IWS
\subsubsection*{Calculating IWS \notedme{Check that you've actually introduced the term `IWS' before you use it! You should check that for all abbreviations.}}
\label{ssc:calc_iws}

Simulations were carried out to replicate intrusion attack at different nodes.
The individual simulation runs were treated in a fashion  similar to building baseline data. \notedme{It's the same, beyond just being similar, no?}
During an update replicating an intrusion, the update time recorded at each of the motes were processed  and mathematically treated according to the utility function in Equation-\ref{eqn2}. The calculation performed the following computations:\notedme{The equation is trivial. You definitely don't need to explain it step by step. The one aspect that you might want to preserve is the notion that the data smoothing point in 2) below. The division by zero avoidance should be obvious to reviewer, but you might want to mention having explicitly formed a `well behaved function'.}
\begin{enumerate}
...
\notedme{I have no idea what you're talking about here, yet...}
...
\notedme{If the related information is just two pieces of information, refer to them directly and avoid the extra `related information' terminology.}
...
The threat model is directed towards injection of unauthorised program image to infect the entire WSN. \notedme{This seems inconsistent with the next sentence. Overall this paragraph is not very clear: you're illustrating what happens to a hypothetical node, but not stating the thread concisely.}
...
For example, when an intrusion takes place in close vicinity of the sink, this IWS may not always be large enough to be identified as an intrusion. 
\notedme{The start of this paragraph is excellently clear, concise and relevant!}
On the other hand, due to node compromise in one of the motes and intrusion initiated from another mote may result into unsurprising score due to fact that the effects may cancel each others contribution to the score.
\notedme{However I have no idea what you're talking about here. How would effects cancel out?}
The deployment should consider additional security measures to handle such scenarios.
\notedme{This sounds weak.}

\subsection{Assumptions} %Should I put it here or in discussion section.
\label{sc:assump}
...
The system is expected to have synchronised clocks.
...
\notedme{You can say what you want about what you expect. However is it reasonable to need synchronised clocks? It depends on the precision of timing information that you need, I suspect...}
...
\notedme{I don't understand what's going on with the text here...}
...
\notedme{If you mean ``in TinyOS'' then you need to say so---the term AM is not something I've heard before...}
...
\notedme{Why isbn't this integrated with the text above?}
...
\notedme{This paragraph reads well to me...}
...
\label{ssc:db}
\notedme{Almost all of the DB material here sees too basic to mention, AFAICS. As a way to consider this, what would the alternative approach be? If there's nothing challenging or interesting in the answer to that question, the database discussion could just be something like ``we store the timing information in a database so as to be able to efficiently query it later.''}
...
Whenever an update takes place in a mote, it reboots with the new software running and stores the boot-up time in its memory.
I NEED TO CONFIRM IF THE TIME IS ALREADY SAVED WITH THE IMAGE AS PART OF DELUGE PROTOCOL IMAGE/GOLDEN IMAGE.
\notedme{I'd guess that it probably isn't...}
The locally stored information  is used later in case it requires to recover from packet delivery failure.
The information $type$, $mote\_id$,  $time\_stamp$,$upd\_seq$ are packed along with an additional checksum in an AM packet\notedme{Why can't you just say `packet'? Why is the AM aspect important? If it is important, include the justification here.} and forwarded to the sink/sinks.
...
\end{verbatim}\notedme{Why not just use something similar to the tabular environment that follows?}
\begin{tabular}{|c|c|c|c|c|}
\hline
Source ID & msgtype & timestamp & upd\_seq & checksum\\
\hline
\end{tabular}
...
%They exhibit highly transient loss patterns that are susceptible to changes in environmental conditions [21]. Asymmetric links are common, and prior work has shown network behaviour to often be worse indoors than out, predominantly due to multi-path effects [22]. 
...
The transmission power level was set at 100\% which allowed a  transmission range of 50 meter and nodes were  able to interfere other nodes at  up to 100 meter apart.
The motes were placed at a spacing of 35 meters along the circumference of the ellipse. 
...
\notedme{OK... but then why are you showing IWS on the same table? IWS makes the table a description of vulnerabilities, not a study of update time statistics... This is not clear enough.}
...
Motes reporting an IWS up to 53.5 would be considered in the GREEN zone.
...
\notedme{`immensely' is too over the top.}
...
%  LocalWords:  Ashraful Alam Contiki Cooja dme
